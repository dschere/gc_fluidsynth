...
// allow for voice data to be routed to a user defined routine.
static void *_user_per_channel_fx_func_data = NULL;
static void (*_user_per_channel_fx_func)(void *userdata, 
    int chan, fluid_real_t* buf, int len) =  NULL; 
    ...
    for(i = 0; i < blockcount; i++)
    {
        ...
        else
        {
            ...
            if(s < FLUID_BUFSIZE)
            {
                /* voice has finished */
                break;
            } else if (_user_per_channel_fx_func != NULL) {
                // I think this is a better place to apply effects than fluid_rvoice_mixer_process_fx()
                // I can map an rvoice back to the channel number by modifying the rvoice structure
                // by adding a field called 'channel' and setting 
                // rvoice->channel inside of the fluid_voice_init() function.   
                
                // static void *_user_per_channel_fx_func_data = NULL;
                // static void (*_user_per_channel_fx_func)(void *userdata, 
                //     int chan, fluid_real_t* buf, int len) =  NULL; 
                _user_per_channel_fx_func(_user_per_channel_fx_func_data,
                     rvoice->chan, &src_buf[FLUID_BUFSIZE * i], s);
            }
        }
    }
...
DECLARE_FLUID_RVOICE_FUNCTION(fluid_rvoice_mixer_set_userfx)
{
// ^^^^ In case you where wondering were param comes from 
// #define DECLARE_FLUID_RVOICE_FUNCTION(name) 
//    void name(void* obj, const fluid_rvoice_param_t param[MAX_EVENT_PARAMS])
    _user_per_channel_fx_func = (PerChannelFxFuncType) param[0].ptr;
    _user_per_channel_fx_func_data = param[1].ptr;
}
